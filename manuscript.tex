...
Consider the task in Figure \ref{fig:experiment}, in which people observe a blue ball moving around in a box, and are then asked, ``will the ball go through the hole?''. How do people make this type of judgment? We hypothesize that they predict where the ball will go by running noisy physical simulations, and that they run a variable number of simulations on each trial according to the \textit{sequential probability ratio test}, or SPRT \cite{wald1947sequential}. In this section, we lay out the formalism for both the simulation-based model and the SPRT strategy.

\subsection{The ``Noisy Newton'' hypothesis}

There is a growing body of evidence that people reason about physical scenes by running noisy simulations. This hypothesis, referred to as the ``Noisy Newton'' hypothesis, states that people have approximate knowledge of physical laws instantiated in a runnable model of intuitive physics. Using this model, they can extrapolate the future of physical scenes by running a series of noisy simulations \cite{Smith2012,Battaglia2013,Smith:2013ug,Smith:2013th,Smith:2014tx,Ullman:2014ut,Hamrick:2015}. In particular, \citeA{Smith2012} investigated the various sources of uncertainty in these simulations, finding that people's judgments were best captured by a model that took into account both perceptual uncertainty and dynamics uncertainty.

Specifically, \citeA{Smith2012} asked people to catch a ball like the one in Figure \ref{fig:experiment} using a paddle that could move up and down along the $y$-axis. In order to predict where people would place the paddle on each trial, their model incorporated uncertainty in its physical simulations through four parameters: perceptual uncertainty over the position of the ball ($\sigma_p$), uncertainty in the initial direction of velocity ($\kappa_v$), ongoing uncertainty in the direction of movement ($\kappa_m$), and uncertainty in the bounce angle ($\kappa_b$).
...
Given the threshold, the probability of taking $N$ samples and then choosing $H_1$ is:
\begin{multline}
...
To combine the ``Noisy Newton'' hypothesis with the SPRT strategy, we needed to compute the probability, $p$, that the ball goes in the hole. To do this, we simulated $M=10000$ samples from the model. From these samples, we estimated a truncated normal distribution along the wall with the hole in it, and then computed the probability density lying within the region of the hole.
...
In order to find the expected number of samples by SPRT, we need to know the probability of the ball going through the hole. We approximated this from the model fits to data from Experiment 2, with one adjustment. Specifically, it is not clear how many samples people would be taking to solve the paddle problem. To resolve this issue, we multiplied the standard deviation calculated from the model by a constant number, since statistics tells us:

sd_prediction = sd_samples / sqrt(n_samples)

This provides one of the two free parameters of the model. Given that adjustment, we can approximate the posterior belief over where the ball will go as a Gaussian, then trivially calculate the probability mass of that Gaussian that lies within each hole. From that, we can get the p(success) for each trial / hole combination

Next, in order to calculate how many samples people will take on average according to SPRT, we need to know the number of samples for threshold - the second free parameter. But based on this threshold, we can just plug in Equation 12 from the one-and-done appendix, and get the average number of samples for each trial / hole combination.

Finally, we can assume that there are some startup costs to simulation, but that each simulation sample should take an additional, constant amount of time. In which case, there should be a linear relationship between the SPRT sample estimates and the reaction times. So I did a grid search to find the set of sd_adj and T parameters that that would maximize the correlation between the two.

The roughly best fitting parameters for this sets the sd adjustment at 0.7 and the SPRT threshold at 2. You might have one obvious complaint, which is that if people are taking multiple samples to solve the paddle task, then the sd adjustment should be positive (since it should be proportional to the square root of the number of samples), and it's not - in fact, positive adjustments make the fit awful. But I think what's actually going on is that the model overestimates the standard deviation, and interestingly, this is almost exactly by a factor of 1 / 0.7. So I think all that this adjustment is doing is correcting for the overestimation in the model.

\section{Does SPRT explain human judgments?}

As we found the number of bounces to be an extremely strong predictor of response time in Experiment 1, we modified our response time equation to incorporate the number of bounces, $B$:
\begin{equation}
t = \beta_0 + (\beta_1 + \beta_2\cdot{}\mathbb{E}[B]) \cdot{}\mathbb{E}[N\,|\,T,p]
\end{equation}
where $\beta_0$, $\beta_1$, and $\beta_2$ are fitted coefficients. \TODO{figure}
