...
Recent research in the domain of physical reasoning has revealed that people make predictions about physical scenes---such as those found in Angry Birds---by running noisy physical simulations \cite{Sanborn2013,Smith:2013fc,Battaglia2013,Smith:2013ug,Smith:2013th,Smith:2014tx,Ullman:2014ut,Hamrick:2015}.
...
Importantly, the SPRT strategy predicts that people need to take more samples---and thus also will take a longer time to respond---on more difficult tasks.
...
In this paper, we first formalize our model, combining the physical prediction model from \citeA{Smith:2013fc} with the SPRT decision strategy. 
...
Because the experiment in Figure \ref{fig:experiment} (see Experiment) was performed online, we additionally performed an online replication of the experiment from \citeA{Smith:2013fc} to set model parameters (see the Appendix for details).
...
In this paper, we asked the question: how do people make best use of their mental simulations?
Specifically, how do people determine how many simulations to run?
We hypothesized that people still vary the number of samples that they take in order to exploit the fact that some judgments are easier to make than others.
